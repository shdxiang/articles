...
收款人等到交易被加到区块中且其后增加了 $z$ 个区块。他不知道攻击者确切的进度，但是假设诚实的块按期望的平均时间生成区块，攻击者潜在进度将是一个 `泊松分布（Poisson distribution）`，其期望值为：
...
占多数 算力 区块 工作量证明 移动平均数 哈希 激励 输出值 输入值 扇出 可信第三方 二项随机漫步 赌徒破产理论 泊松分布 泊松密度
